...
class PingPongActorSpec(_system: ActorSystem) extends TestKit(_system) with ImplicitSender
  ...
  "An IndexerActor" must {
    "index the document passed" in {
      val actor = system.actorOf(IndexerActor.props)
      actor ! Document("ABC", "document content")
    }
  }

  "An BrokerActor" must {
    "react to a FeedMe request" in {
      val actor = system.actorOf(BrokerActor.props)
      actor ! BrokerActor.FeedMe
    }
  }

  "An FetcherActor" must {
    "react to a FeedMe request" in {
      val actor = system.actorOf(FetcherActor.props)
      actor ! FetcherActor.FeedMe
    }
  }
}
